...
import { TYPE_LABEL, voteScore, isReady, READY_THRESHOLD, type Proposal } from "@/lib/werkstatt-meta";

const sel = "w-full rounded-lg border border-line bg-surface-2 px-3 py-2 text-sm text-ink outline-none focus:border-accent";

export function ProposalDetail({ initial, me, admin }: { initial: Proposal; me: string; admin: boolean }) {
  ...
  const myVote = p.votes?.[me] || 0;
  const score = voteScore(p);
  const decided = p.status !== "discussion";
  ...
  async function call(path: string, body: Record<string, unknown>) {
    ...
    try {
      ...
      else alert(j.error === "forbidden" ? "Keine Berechtigung." : "Aktion fehlgeschlagen.");
    } catch { alert("Verbindungsfehler."); }
    setBusy(false);
  }
  const vote = (v: number) => call("vote", { value: myVote === v ? 0 : v });
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-xl border border-line bg-surface p-5 shadow-sm">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          ...
          <StatusBadge p={p} />
          {isReady(p) && <span className="rounded-full bg-amber/15 px-2 py-0.5 text-[10px] font-semibold text-amber">bereit zur Freigabe</span>}
        </div>
        <h1 className="text-xl font-extrabold">{p.title}</h1>
        <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-ink">{p.content}</p>

        {/* Voting */}
        <div className="mt-4 flex items-center gap-2">
          <button disabled={busy || decided} onClick={() => vote(1)} className={cn("inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-semibold transition disabled:opacity-50", myVote === 1 ? "bg-green text-bg" : "bg-surface-2 text-muted hover:text-ink")}><Icon name="check" className="h-4 w-4" />Dafür</button>
          <button disabled={busy || decided} onClick={() => vote(-1)} className={cn("inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-semibold transition disabled:opacity-50", myVote === -1 ? "bg-red text-bg" : "bg-surface-2 text-muted hover:text-ink")}><Icon name="x" className="h-4 w-4" />Dagegen</button>
          <span className="ml-1 text-sm text-muted-2">Netto: <b className="text-ink">{score >= 0 ? "+" : ""}{score}</b> · ab +{READY_THRESHOLD} bereit</span>
        </div>
      </div>
        ...
        <div className="rounded-xl border border-accent/40 bg-accent/5 p-4 shadow-sm">
          ...
          {!adapt ? (
            <div className="flex flex-wrap gap-2">
              <button disabled={busy} onClick={() => decide("approved")} className="rounded-lg bg-green px-3 py-1.5 text-sm font-semibold text-bg disabled:opacity-50">Annehmen{p.type === "einwand" ? " & übernehmen" : ""}</button>
              <button disabled={busy} onClick={() => setAdapt({ title: p.title, content: p.content })} className="rounded-lg bg-surface-2 px-3 py-1.5 text-sm font-semibold hover:text-ink disabled:opacity-50">Anpassen & übernehmen</button>
              <button disabled={busy} onClick={() => decide("rejected")} className="rounded-lg bg-red/10 px-3 py-1.5 text-sm font-semibold text-red hover:bg-red/20 disabled:opacity-50">Ablehnen</button>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <button disabled={busy} onClick={() => decide("adapted", { title: adapt.title, content: adapt.content })} className="rounded-lg bg-accent px-3 py-1.5 text-sm font-semibold text-bg disabled:opacity-50">Übernehmen</button>
                <button onClick={() => setAdapt(null)} className="rounded-lg bg-surface-2 px-3 py-1.5 text-sm font-semibold">Abbrechen</button>
              </div>
            </div>
          )}
      ...
      {/* Kommentare */}
      <div className="rounded-xl border border-line bg-surface p-4 shadow-sm">
        <h2 className="mb-3 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-muted-2"><Icon name="chat" className="h-3.5 w-3.5" />Diskussion ({p.comments.length})</h2>
        <ul className="mb-3 flex flex-col gap-2">
          {p.comments.map((c) => (
            <li key={c.id} className="rounded-lg border border-line bg-surface-2/40 p-2.5 text-sm">
              <div className="mb-0.5 text-[11px] text-muted-2">{c.user_key === me ? "Du" : "Kolleg:in"} · {c.at.slice(0, 10)}</div>
