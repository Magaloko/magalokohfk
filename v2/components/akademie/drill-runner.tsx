  ...
  return (
    <Modal onClose={onClose}>
      <div className="mb-3 flex items-start justify-between gap-3">
        <h3 className="text-base font-bold">⚡ Drill-Training</h3>
        ...
        <div className="mt-3">
          ...
          {answered !== correctIdx && q.opts[correctIdx]?.feedback && (
            <div className="mt-2 rounded-lg border-l-2 border-green bg-surface-2 px-3 py-2 text-sm text-muted">
              ✓ Richtig ({String.fromCharCode(65 + correctIdx)}): {q.opts[correctIdx].feedback}
            </div>
          )}
          {q.muster && <div className="mt-2 rounded-lg border-l-2 border-accent bg-surface-2 px-3 py-2 text-sm text-muted">💡 Musterantwort: {q.muster}</div>}
