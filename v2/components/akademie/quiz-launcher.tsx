  ...
  return (
    <>
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setCfg({ n: 5, drills, einwaende, marken })} className={`${btn} bg-accent text-bg hover:opacity-90 flex items-center gap-1.5`}><Icon name="target" className="h-4 w-4" /> Schnell-Quiz (5)</button>
        ...
        {has.drills && <button onClick={() => setCfg({ n: 5, drills, einwaende: [], marken: [] })} className={`${btn} bg-surface-2 hover:text-ink flex items-center gap-1.5`}><Icon name="bolt" className="h-4 w-4" /> Nur Übungen</button>}
