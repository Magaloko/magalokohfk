...
import { leverScore, formatEur, isLeverActive, type Lever } from "@/lib/cockpit";
...
export default async function HebelPage() {
  ...
  const href = (r: Lever) => `/mago/hebel/${encodeURIComponent(r.id || String(levers.indexOf(r)))}`;
  ...
  return (
    <PageShell icon="lever" title="Hebel" subtitle={`${levers.filter(isLeverActive).length} aktiv · sortiert nach ROI`}
      action={<div className="flex gap-2">
        <Link href="/mago/hebel/board" className="inline-flex items-center gap-1.5 rounded-lg bg-surface-2 px-3 py-2 text-sm font-semibold hover:text-ink"><Icon name="cockpit" className="h-4 w-4" />Board</Link>
