...
import { leverScore, formatEur, type Lever } from "@/lib/cockpit";
...
const Back = <Link href="/mago/hebel" className="rounded-lg bg-surface-2 px-3 py-1.5 text-sm font-semibold text-muted hover:text-ink">← Hebel</Link>;
...
export default async function HebelDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const history = await getRecordHistory("magoHebel", l.id || key, LEVER_FIELDS);
  ...
  return (
    <PageShell title={l.title || "Hebel"} subtitle={l.area || undefined} action={Back}>
      <div className="flex flex-col gap-4">
        ...
        <LeverActions collection="magoHebel" backHref="/mago/hebel" id={l.id || key} lever={l} />
