...
import { leverScore, formatEur } from "@/lib/cockpit";
import { getMagoLevers } from "@/lib/mago";
...
export default async function HebelBoardPage() {
  ...
  const levers = await getMagoLevers();
  const cards: KanbanCard[] = levers.map((l, i) => ({
    id: l.id || String(i),
    title: l.title || "Hebel",
    status: l.status || "Backlog",
    href: `/mago/hebel/${encodeURIComponent(l.id || String(i))}`,
  ...
  return (
    <PageShell icon="lever" title="Hebel-Board" subtitle="Pipeline nach Status — Karten per Drag & Drop verschieben"
      action={<div className="flex gap-2">
        <Link href="/mago/hebel" className="inline-flex items-center gap-1.5 rounded-lg bg-surface-2 px-3 py-2 text-sm font-semibold hover:text-ink"><Icon name="kpi" className="h-4 w-4" />Liste</Link>
        <NewLeverButton collection="magoHebel" />
      </div>}>
      <KanbanBoard collection="magoHebel" statuses={LEVER_STATUSES} cards={cards} />
    </PageShell>
  );
}
