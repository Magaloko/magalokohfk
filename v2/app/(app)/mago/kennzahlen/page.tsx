...
import { sortedWeeks } from "@/lib/cockpit";
import { getMagoKpis } from "@/lib/mago";
...
export default async function KennzahlenPage() {
  ...
  const weeklyKpis = await getMagoKpis();
  const weeks = sortedWeeks(weeklyKpis).slice(0, 12);

  if (!weeks.length) return <PageShell icon="kpi" title="KPIs" action={<NewKpiButton collection="magoKpis" />}><EmptyState title="Noch keine KPI-Wochen" hint="Wöchentliche Kennzahlen erfassen." /></PageShell>;

  return (
    <PageShell icon="kpi" title="KPIs" subtitle={`${weeklyKpis.length} erfasste Wochen`} action={<NewKpiButton collection="magoKpis" />}>
        ...
        {weeks.map((w, i) => {
          ...
          return (
            <section key={w.id || i} className="rounded-xl border border-line bg-surface p-4 shadow-sm">
              ...
              <KpiActions collection="magoKpis" id={w.id || String(weeklyKpis.indexOf(w))} week={w} />
            </section>
          );
        })}
      </div>
