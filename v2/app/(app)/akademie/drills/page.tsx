...
export default async function DrillsPage() {
  ...
  return (
    <PageShell title="Tagesübungen" icon="bolt" subtitle={`${d.drills.length} Übungen · tägliches Mikro-Training`} action={admin ? <NewDrillButton /> : undefined}>
      <DrillLauncher drills={d.drills} />
      <QuizLauncher drills={d.drills} einwaende={d.einwaende} marken={d.marken} />
      <CollapsibleOnMobile title={`Alle ${d.drills.length} Übungen`}>
        <DataTable columns={cols} rows={d.drills} getKey={(r, i) => r.id || String(i)} empty={{ title: "Noch keine Übungen", hint: "Lernsystem importieren." }} />
      </CollapsibleOnMobile>
    </PageShell>
  );
}
