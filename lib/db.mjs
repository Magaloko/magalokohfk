...
// === Rollenbasierter State-Filter (portiert) ===
...
const TG_PRODUKT_KEYS = ["vipArticles", "jtlManufacturers", "jtlSuppliers", "sortimentRules", "sortimentStats"];

export function filterStateForTgRole(full, modules) {
  const allowed = new Set(TG_AKADEMIE_KEYS);
  if (Array.isArray(modules) && modules.includes("produkt")) TG_PRODUKT_KEYS.forEach((k) => allowed.add(k));
